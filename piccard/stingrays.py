...
# This file contains the older versions of the stingray transformation. They
# were all testing versions: basic attempts to avoid having to do the full
# stingray transform to gain computational efficiency. Unfortunately, they all
# suffer from degeneracy problems that thwart proper exploration of the
# parameter space.
...
class tmStingrayLikelihood(stingrayLikelihood):
    ...
    def stingray_hessian_quants(self, p, set_hyper_pars=True):
        ...
        raise NotImplementedError("Hessian not implemented for this class!")
    ...
    def stingray_hessian_quants(self, p, set_hyper_pars=True):
        ...
        for ii, psr in enumerate(self.ptapsrs):
            ...
            if psr.fourierind is not None:
                ...
                # Only red-noise -- second derivatives
                for psra in self.ptapsrs:
                    for key, d2_Phivec_d2_p in psra.d2_Phivec_d2_param.iteritems():
            ...
            if psr.dmfourierind is not None:
                ...
                # Second derivatives
                for psra in self.ptapsrs:
                    for key, d2_Thetavec_d2_p in psra.d2_Thetavec_d2_param.iteritems():
                        if key[0] == key[1]:
                            # Log-jacobian
                            hessian[key[0], key[1]] += np.sum(d2_theta_d2_theta *
                                    d2_Thetavec_d2_p)
