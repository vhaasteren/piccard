...
class ptaPulsar(object):
    ...
    def __init__(self):
        ...
        # Indices for when we are in mark13
...
class ptaLikelihood(object):
    ...
    def clear(self):
        ...
        self.freqpinds = None    # Which parameter indices belong to the modes
        self.freqpfnds = None    # Which frequency indices belong to the modes
    ...
    def allocateLikAuxiliaries(self):
        ...
        # These quantities are mostly used in explicit Fourier mode sampling,
        # for when we are numerically dealing with the quadratic coefficients.
        maxfreqs = np.max(self.npf)
        self.freqmask = np.zeros((npsrs, maxfreqs), dtype=np.bool)
        self.freqpinds = np.zeros((npsrs, maxfreqs), dtype=np.int)
        self.freqfinds = np.zeros((npsrs, maxfreqs), dtype=np.int)
        self.freqb = np.zeros((npsrs, maxfreqs))

        for jj, psr in enumerate(self.ptapsrs):
            pind = psr.fourierind
            find = np.sum(self.npf[:jj])
            nfreq = self.npf[jj]

            self.freqmask[jj, :nfreq] = True  # No npff here?
            self.freqfinds[jj, :nfreq] = np.arange(find, find+nfreq)

            if pind is not None:
                self.freqpinds[jj, :nfreq] = np.arange(pind, pind+nfreq)
    ...
    def makeModelDict(self,  nfreqs=20, ndmfreqs=None, \
            ...
            gwbSi=4.33, gwbVarySi=True, gwbPrior='flat', \
        ...
        for pp, m2psr in enumerate(self.ptapsrs):
            ...
            if incTimingModel and not likfunc in ['mark13', 'mark14', 'mark15']:
                ...
                if keepTimingModelPars is None and not likfunc in ['mark13',
                    ...
                    # Actually, if we are in likfunc == 'mark13', we need _all_
            ...
            if likfunc in ['mark13', 'mark14', 'mark15']:
                ...
                if 'jitter' in gibbsmodel:
                    # We already have the U-matrix. Use those modes
                    nmodes = len(m2psr.avetoas)
                    bvary = [True]*nmodes
                    pmin = [-1.0e6]*nmodes
                    pmax = [1.0e6]*nmodes
                    pstart = [1.0e-9]*nmodes
    ...
    def smBpsd(self, lAmp, alpha, lfc, T, freqs):
        """given the signal parameters, return the PSD as placed on the diagonal
        of the prior covariance
        """
        fc = 10**lfc / pic_spy
    ...
    def d_smBpsd(self, lAmp, alpha, lfc, T, freqs, ntotfreqs=None, nfreqind=None):
        ...
        d_mat = np.zeros((len(freqs), 3))

        d_mat[nfreqind:nfreqind+len(freqs),0] = (np.log(10)*10**lAmp * pic_spy**3 / T) * ((1 + (freqs/fc)**2)**(-0.5*alpha))
        raise NotImplemented("Spectral model gradient not completed yet!")
        d_mat[nfreqind:nfreqind+len(freqs),1] = (10**lAmp * pic_spy**3 / T) * ((1 + (freqs/fc)**2)**(-0.5*alpha))
        d_mat[nfreqind:nfreqind+len(freqs),2] = (10**lAmp * pic_spy**3 / T) * ((1 + (freqs/fc)**2)**(-0.5*alpha))
    ...
    def constructPhiAndTheta(self, parameters, selection=None, \
        ...
        for ss in np.hstack((self.sig_Phi_inds,
            ...
            if selection[int(ss)]:
                ...
                psrind = m2signal['pulsarind']
                psr = self.ptapsrs[psrind]

                if m2signal['stype'] == 'spectrum':
                    if m2signal['corr'] == 'single':
                        findex = np.sum(self.npff[:psrind])
                        ...
                        if calc_gradient:
                            ...
                            psr.add_d_Phivec(d_mat, m2signal['parindex'],
                                    m2signal['bvary'])
                        ...
                        if calc_hessian:
                            ...
                            d_mat = self.d2_spBpsd(sparameters,
                                    ntotfreqs=ntotfreqs, nfreqind=findex)
                            psr.add_d2_Phivec(d_mat, m2signal['parindex'],
                                    m2signal['bvary'])
                    ...
                    inds = np.sum(self.npfb[:psrind])
                    inde = inds + self.npfb[psrind]

                    # Find the FB index for the band we are in
                    bind = np.where(np.all(psr.Fbands==m2signal['freqband'],axis=1))[0][0]
                    findex = np.sum(self.npfb[:psrind]) + bind*self.npf[psrind]
                    nfreq = int(self.npf[psrind]/2)
                    ...
                    if calc_gradient:
                        d_mat = self.d_spBpsd(sparameters,
                                len(psr.Fbands)*self.npf[psrind],
                                bind*self.npf[psrind])
                        psr.add_d_Betavec(d_mat, m2signal['parindex'],
                                m2signal['bvary'])
                    ...
                    if calc_hessian:
                        d_mat = self.d2_spBpsd(sparameters,
                                len(psr.Fbands)*self.npf[psrind],
                                bind*self.npf[psrind])
                        psr.add_d2_Betavec(d_mat, m2signal['parindex'],
                                m2signal['bvary'])
                elif m2signal['stype'] == 'dmspectrum':
                    if m2signal['corr'] == 'single':
                        findex = np.sum(self.npffdm[:psrind])
                        nfreq = int(self.npfdm[psrind]/2)
                        ...
                        if calc_gradient:
                            d_mat = self.d_spBpsd(sparameters)
                            psr.add_d_Thetavec(d_mat, m2signal['parindex'],
                                    m2signal['bvary'])

                        if calc_hessian:
                            d_mat = self.d2_spBpsd(sparameters)
                            psr.add_d2_Thetavec(d_mat, m2signal['parindex'],
                                    m2signal['bvary'])
                elif m2signal['stype'] == 'powerlaw':
                    lAmp = sparameters[0]
                    Si = sparameters[1]
                    sTmax = m2signal['Tmax']
                    sfreqs = self.ptapsrs[psrind].Ffreqs
                    ...
                    Si = sparameters[1]
                    sTmax = m2signal['Tmax']
                    sfreqs = self.ptapsrs[m2signal['pulsarind']].Fdmfreqs
    ...
    def updateDetSources(self, parameters, selection=None, calc_gradient=False):
        """
            ...
            if selection[ss]:
                ...
                if m2signal['stype'] == 'lineartimingmodel':
                    ...
                    if len(newdes) == psr.Mmat.shape[1]:
                        # This is a speed hack, necessary for mark13
                        Mmat = psr.Mmat
                    else:
    ...
    def mark13loglikelihood(self, parameters, set_hyper_pars=True):
        ...
        for ii, psr in enumerate(self.ptapsrs):
            ...
            if psr.fourierind is not None:
                ...
                # We include Svec, but not correlations...
                bsqr = parameters[pslc]**2
                phivec = self.Phivec[fslc] + self.Svec[:nfreq]
                ...
                for key, d_Phivec_d_p in psr.d_Phivec_d_param.iteritems():
                    # Inner product
                    gradient[key] += 0.5 * np.sum(bsqr * d_Phivec_d_p[fslc] / phivec**2)

                    # Determinant
                    gradient[key] -= 0.5 * np.sum(d_Phivec_d_p[fslc] / phivec)

                # Gradient for Svec hyper-parameters
                for key, d_Svec_d_p in self.d_Svec_d_param.iteritems():
                    # Inner product
                    gradient[key] += 0.5 * np.sum(bsqr * d_Svec_d_p[fslc] / phivec**2)

                    # Determinant
                    gradient[key] -= 0.5 * np.sum(d_Svec_d_p[fslc] / phivec)
    ...
    def mark14hessian(self, parameters, set_hyper_pars=True):
        ...
        for ii, psr in enumerate(self.ptapsrs):
            ...
            if psr.dmfourierind is not None:
                ...
                # Second derivatives
                for key, d2_Thetavec_d2_p in psr.d2_Thetavec_d2_param.iteritems():
                    if key[0] == key[1]:
                        # Inner product
    ...
    def loglikelihood(self, parameters):
        ...
        if(np.all(self.pmin <= parameters) and np.all(parameters <= self.pmax)):
            if self.likfunc == 'mark1':
                ...
                ll, _ = self.mark13loglikelihood(parameters)
    ...
    def mark13logprior(self, parameters):
        lp = 0.0
    ...
    def logprior(self, parameters):
        ...
        elif self.likfunc in ['mark9', 'mark10']:
    ...
    def addPriorDraws(self, which='hyper'):
        ...
        self._prior_draw_signal = list(np.unique(self._prior_draw_signal))
