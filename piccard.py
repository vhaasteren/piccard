...
"""
A general signal element of the mark3 model/likelihood.

The mark2 model/likelihood was not using any data compression. When dealing with
large datasets with multiple efac/equad combinations, handling such dataset
efficiently can be troublesome. The mark3 model/likelihood uses data compression
to solve this.

The efac/equad values can be varied in the mcmc, but they are most likely better
held fixed. Use the mark2 formalism to determine these quantities, and fix them
in mark3. This dramatically reduces the dimensionality of the problem.

Note: at the moment still a copy of mark2...
"""
class mark3signal(object):
    pulsarind = None        # pulsar nr. for EFAC/EQUAD
    stype = "none"          # EFAC, EQUAD, spectrum, powerlaw
    corr = "single"         # single, gr, uni, dipole...
...
class mark3Likelihood(object):
    ...
    def initFromFile(self, filename):
        ...
        for psrname in psrnames:
            newpsr = mark3Pulsar()
            newpsr.readFromH5(filename, psrname)
            self.m3psrs.append(newpsr)

    # Initialise the model
    def initModel(self, nfreqmodes=20):
        # For every pulsar, construct the auxiliary quantities like the Fourier
        # design matrix etc
        if len(self.m3psrs) < 1:
            raise IOError, "no pulsars found in hdf5 file"

        Tstart = np.min(self.m3psrs[0].toas)
        Tfinish = np.max(self.m3psrs[0].toas)

        for m3psr in self.m3psrs:
            Tstart = np.min([np.min(self.m3psrs[0].toas), Tstart])
            Tfinish = np.max([np.max(self.m3psrs[0].toas), Tfinish])

        # Total duration of the experiment
        Tmax = Tfinish - Tstart
        for m3psr in self.m3psrs:
            m3psr.createAuxiliaries(nfreqmodes, Tmax)

        # Initialise the mark3signal objects
        # Currently: one efac per pulsar, and red noise
        self.m3signals = []
        index = 0
        for ii in range(len(self.m3psrs)):
            # Create an efac signal
            newsignal = mark3signal()
            newsignal.pulsarind = ii
            newsignal.stype = 'efac'
            newsignal.corr = 'single'
            N = np.diag(self.m3psrs[ii].toaerrs**2)
            newsignal.GNG = np.dot(self.m3psrs[ii].Gmat.T, np.dot(N, self.m3psrs[ii].Gmat))
            newsignal.npars = 0
            newsignal.nindex = index
            index += newsignal.npars
            ...
            newsignal.GNGldet = 2*np.sum(np.log(np.diag(cf[0])))
            GNGinv = sl.cho_solve(cf, np.identity(newsignal.GNG.shape[0]))
            newsignal.GGNGG = np.dot(self.m3psrs[ii].Gmat, np.dot(GNGinv, self.m3psrs[ii].Gmat.T))
            newsignal.rGGNGGr = np.dot(self.m3psrs[ii].residuals, np.dot(newsignal.GGNGG, self.m3psrs[ii].residuals))
            newsignal.rGGNGGF = np.dot(self.m3psrs[ii].Fmat.T, np.dot(newsignal.GGNGG, self.m3psrs[ii].residuals))
            newsignal.FGGNGGF = np.dot(self.m3psrs[ii].Fmat.T, np.dot(newsignal.GGNGG, self.m3psrs[ii].Fmat))

            self.m3signals.append(newsignal)
            ...
            newsignal.stype = 'powerlaw'
            newsignal.corr = 'single'
            #newsignal.npars = int(len(self.m3psrs[ii].Ffreqs)/2)
            newsignal.npars = 2
            newsignal.Tmax = Tmax
            newsignal.nindex = index
            self.m3signals.append(newsignal)
        ...
        newsignal.stype = 'powerlaw'
        newsignal.corr = 'gr'
        #newsignal.npars = int(len(self.m3psrs[0].Ffreqs)/2)     # Check how many
        newsignal.npars = 2
        newsignal.nindex = index
        newsignal.Tmax = Tmax
        newsignal.hdmat = hdcorrmat(self.m3psrs)           # The H&D matrix
        self.m3signals.append(newsignal)
        index += newsignal.npars

    def initPrior(self):
        self.dimensions = 0
        for m3signal in self.m3signals:
            self.dimensions += m3signal.npars
        ...
        index = 0
        for m3signal in self.m3signals:
            if m3signal.stype == 'efac':
                if m3signal.npars == 1:
                    self.pmin[index] = 0.001
                    self.pmax[index] = 1000.0
                    self.pwidth[index] = 0.1
                    self.pstart[index] = 1.0
                    index += m3signal.npars
            elif m3signal.stype == 'equad':
                self.pmin[index] = -10.0        # 1e-10 secs
                self.pmax[index] = -2           # 1e-2 secs
                self.pwidth[index] = 0.1
                self.pstart[index] = -8.0
                index += m3signal.npars
            elif m3signal.stype == 'spectrum':
                npars = m3signal.npars
                self.pmin[index:index+npars] = -25.0
                self.pmax[index:index+npars] = 10.0
                self.pstart[index:index+npars] = -10.0
                self.pwidth[index:index+npars] = 0.1
                index += m3signal.npars
            elif m3signal.stype == 'powerlaw':
                self.pmin[index:index+2] = [-17.0, 1.02]
                self.pmax[index:index+2] = [-10.0, 6.98]
                self.pstart[index:index+2] = [-15.0, 2.01]
                self.pwidth[index:index+2] = [0.1, 0.1]
    ...
    def m3loglikelihood(self, parameters):
        ...
        # First figure out how large we have to make the arrays
        npsrs = len(self.m3psrs)
        npf = np.zeros(npsrs, dtype=np.int)
        for ii in range(npsrs):
            npf[ii] = len(self.m3psrs[ii].Ffreqs)
        ...
        for m3signal in self.m3signals:
            if m3signal.stype == 'efac':
                pefac = 1.0
                if m3signal.npars == 1:
                    pefac = parameters[m3signal.nindex]

                findex = np.sum(npf[:m3signal.pulsarind])
                nfreq = npf[m3signal.pulsarind]/2

                rGr[m3signal.pulsarind] = m3signal.rGGNGGr / pefac
                rGF[findex:findex+2*nfreq] = m3signal.rGGNGGF / pefac
                GNGldet[m3signal.pulsarind] = m3signal.GNGldet + np.log(pefac) * self.m3psrs[m3signal.pulsarind].Gmat.shape[1]
                FGGNGGF[findex:findex+2*nfreq, findex:findex+2*nfreq] += m3signal.FGGNGGF / pefac
            elif m3signal.stype == 'equad':
                # TODO: Change this from an efac, now is the same
                findex = np.sum(npf[:m3signal.pulsarind])
                nfreq = npf[m3signal.pulsarind]/2

                rGr[m3signal.pulsarind] = m3signal.rGGNGGr / parameters[m3signal.nindex]
                rGF[findex:findex+2*nfreq] = m3signal.rGGNGGF / parameters[m3signal.nindex]
                GNGldet[m3signal.pulsarind] = m3signal.GNGldet + np.log(parameters[m3signal.nindex]) * self.m3psrs[m3signal.pulsarind].Gmat.shape[1]
                FGGNGGF[findex:findex+2*nfreq, findex:findex+2*nfreq] += m3signal.FGGNGGF / parameters[m3signal.nindex]
            elif m3signal.stype == 'spectrum':
                if m3signal.corr == 'single':
                    findex = np.sum(npf[:m3signal.pulsarind])
                    nfreq = npf[m3signal.pulsarind]/2

                    # Pcdoubled is an array where every element of the parameters
                    # of this m3signal is repeated once (e.g. [1, 1, 3, 3, 2, 2, 5, 5, ...]
                    pcdoubled = np.array([parameters[m3signal.nindex:m3signal.nindex+m3signal.npars], parameters[m3signal.nindex:m3signal.nindex+m3signal.npars]]).T.flatten()

                    # Fill the phi matrix
                    di = np.diag_indices(2*nfreq)
                    Phi[findex:findex+2*nfreq, findex:findex+2*nfreq][di] += 10**pcdoubled
                elif m3signal.corr == 'gr':
                    nfreq = m3signal.npars
                    ...
                    for aa in range(npsrs):
                        for bb in range(npsrs):
                            ...
                            nof = np.min([npf[aa], npf[bb], 2*nfreq])
                            di = np.diag_indices(nof)
                            Phi[indexa:indexa+nof,indexb:indexb+nof][di] += 10**pcdoubled[:nof] * m3signal.hdmat[aa, bb]
                            indexb += npf[bb]
                        indexb = 0
                        indexa += npf[aa]
            elif m3signal.stype == 'powerlaw':
                spd = 24 * 3600.0
                spy = 365.25 * spd
                Amp = 10**parameters[m3signal.nindex]
                Si = parameters[m3signal.nindex+1]

                if m3signal.corr == 'single':
                    findex = np.sum(npf[:m3signal.pulsarind])
                    nfreq = npf[m3signal.pulsarind]/2
                    freqpy = self.m3psrs[m3signal.pulsarind].Ffreqs * spy
                    pcdoubled = (Amp**2 * spy**3 / (12*np.pi*np.pi * m3signal.Tmax)) * freqpy ** (-Si)

                    # Fill the phi matrix
                    di = np.diag_indices(2*nfreq)
                    Phi[findex:findex+2*nfreq, findex:findex+2*nfreq][di] += pcdoubled
                elif m3signal.corr == 'gr':
                    freqpy = self.m3psrs[0].Ffreqs * spy
                    pcdoubled = (Amp**2 * spy**3 / (12*np.pi*np.pi * m3signal.Tmax)) * freqpy ** (-Si)
                    nfreq = len(freqpy)
                    ...
                    for aa in range(npsrs):
                        for bb in range(npsrs):
                            ...
                            di = np.diag_indices(nof)
                            Phi[indexa:indexa+nof,indexb:indexb+nof][di] += pcdoubled[:nof] * m3signal.hdmat[aa, bb]
                            indexb += npf[bb]
                        indexb = 0
                        indexa += npf[aa]
